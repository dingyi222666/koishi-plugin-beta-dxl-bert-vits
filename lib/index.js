...
<tbody>
` + Object.entries(SpeakerKeyIdMap).map((s) => `<tr><td>${s[1]}</td><td>${s[0]}</td></tr>
`).join("") + `
</tbody>
</table>
</details>
...
var Config = import_koishi2.Schema.intersect([
  import_koishi2.Schema.object({
    speaker: import_koishi2.Schema.union(Object.values(SpeakerKeyIdMap)).description("全局默认讲者`有一些可能失效了`<br>可以输入speaker_id匹配查找").default("向晚_ZH"),
  ...
  import_koishi2.Schema.object({
    groupListmapping: import_koishi2.Schema.array(import_koishi2.Schema.object({
...
function apply(ctx, config) {
  ...
  ctx.command("betavits <text:text>", "AIbetavits语音合成帮助").option("speaker", "-s [speaker:string] 语音合成的讲者", {
    ...
    const groupConfig = config.groupListmapping.find((group) => group.groupList === session.channelId);
    const finalSpeaker = groupConfig ? groupConfig.defaultspeaker : config.speaker;
    if (config.loggerinfo) {
      logger.info(`当前频道: ${session.channelId}`);
      if (groupConfig) {
        logger.info(`找到的群组配置: ${JSON.stringify(groupConfig)}`);
      } else {
    ...
    return await vits.say(text, { ...options, speaker: finalSpeaker });
  });
}
__name(apply, "apply");
